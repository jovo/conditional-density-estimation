...
The result of this partitioning is that we can approximate $F_{Y|X}$ at each scale $j$ via a mixture over densities, $\mh{F}^j_{Y|X} \propto \sum_{k \in \mc{K}_j} \pi_{j,k} \mh{F}^{j,k}_{Y|X}$. 
% \dd{change in \begin{equation}F_{Y|X} \approx \mh{F}_{Y|X}= \sum_{j \in \ZZ} \pi_{j,k_j(X)} \mh{F}_{j,k_j(X)} \label{eq:base}\end{equation}}. 
