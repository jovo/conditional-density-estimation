...
(ii) run the Gibbs sampler for $S$ iterations, and at the $s$th  iteration:
 ...
 \hspace{5pt} a) sample parameters $\{\sigma^{(s)}_{j,k_j}, \mu^{(s)}_{j,k_j}, \pi^{(s)}_{j,k_j} \}_{j\in\ZZ,k_j \in\mc{K}_j}$ from its posterior, i.e.  $p(.| x^{(n)},y^{(n)})$
   
 \hspace{5pt}   b) sample $\hat{y}^*_{s}$ from the following mixture
 $$ \sum_{j \in \ZZ} \pi^{(s)}_{j,k_j(x^{*})} \mc{N}\left(\mu^{(s)}_{j,k_j(x^{*})},\sigma^{(s)}_{j,k_j(x^{*})}\right)$$
   
 (iii) given sequence $\left\lbrace  \hat{y}^*_{s}\right\rbrace_{s=1}^S$, summaries of the predictive density such as mean, variance and quantiles can be computed.
...
As we are unaware of other methods, even frequentist, that estimate posteriors with such high-dimensional predictors, we compare point estimates of our approach with other moderately regression algorithms.  In particular, we elected to compare against lasso, classification and regression trees (CART), Random Forest (RF) and principal component (PC) regression. In particular, the lasso regularization parameter and the number of principal components for PC regression were chosen based on  the Akaike information criterion (AIC). For all algorithms, standard Matlab packages were utilized.

\subsection{Additional results}
Tables \ref{table:linear1}, \label{table:mfa} and \label{table:swiss} show mean squared errors and CPU usage under MSB, CART and lasso based on leave-one-out prediction. In particular, table \ref{table:linear1} shows results concerning the linear subspace in section 4.4 for different number of factors, i.e. $d=5$ and $d=10$. Table \ref{table:mfa} shows results concerning the union of linear subspace in section 4.4. for different number of mixture components, i.e. $G=5$ and $G=10$.As shown, in almost all data scenario, our model is able to perform as well as or better than the model associated to the lowest mean squared error and can scale substantially better than others to high dimensional predictors. 
...
\begin{table}[t]
\caption{Non-linear manifold - Swissroll: Mean and standard deviations of squared errors under multiscale stick-breaking (MSB), CART and Lasso for different sample sizes for different simulation scenarios.}\label{table:swiss}
\vskip 0.15in
\begin{center}
\begin{small}
\begin{sc}
\begin{tabular}{lllcccccc}
\hline
&&&\multicolumn{3}{c}{Swissroll}\\

$p$&$n$& & msb&cart& lasso\\
\\
\multirow{3}{*}{$10k$}&\multirow{3}{*}{100}&mse &$0.25$&$0.46$&$0.38$\\
&&std & $0.24$ & $0.53$&$0.40$\\
&&time & 5& $5$&$1$ \\
